...
export class ReservationService {
  ...
  async createReservation(data: {
    ...
    if (data.status !== "canceled") {
      await this.updateReservationQuantitiesCanteen(
        data.mealId,
        data.refeitorioId,
        data.quantity,
        "add",
  ...
  async updateStatus(
    id: number,
    status: "canceled" | "active" | "consumed" | "not consumed" | "pendent",
  ) {
  ...
  async markUnconsumedReservations(targetDate?: Date) {
    ...
    console.log(
      `🔍 [DEBUG] Data formatada: ${checkDate.toLocaleDateString("pt-PT")}`,
    );
    ...
    console.log(
      `🔍 [DEBUG] Total de reservas ativas encontradas: ${allActiveReservations.length}`,
    );

    // Filtrar apenas as que têm data da refeição na data especificada
    const targetReservations = allActiveReservations.filter(
      (reservation: any) => {
        if (!reservation.meal || !reservation.meal.date) {
          console.log(
            `🔍 [DEBUG] Reserva ${reservation.id} sem meal ou meal.date`,
          );
          return false;
        }
        const mealDate = new Date(reservation.meal.date);
        const mealDateOnly = new Date(
        ...
        if (matches) {
          console.log(
            `🔍 [DEBUG] Reserva com o ID ${reservation.id} MATCH! Meal date: ${mealDateOnly.toLocaleDateString("pt-PT")}, Check date: ${checkDate.toLocaleDateString("pt-PT")}`,
          );
        } else {
          console.log(
            `🔍 [DEBUG] Reserva com o ID${reservation.id} não match. Meal date: ${mealDateOnly.toLocaleDateString("pt-PT")}, Check date: ${checkDate.toLocaleDateString("pt-PT")}`,
          );
        }
    ...
    console.log(
      `🔍 [DEBUG] Reservas que correspondem à data: ${targetReservations.length}`,
    );

    let updatedCount = 0;
    for (const reservation of targetReservations) {
      console.log(
        `🔄 [DEBUG] Marcando reserva ${reservation.id} como "not consumed"`,
      );
