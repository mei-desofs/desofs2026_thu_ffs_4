...
export class ReservationQuantitiesCanteenService {
  async getCanteenProductionStatistics(
    canteenId: number,
    filters?: {
      date?: string;
      ...
      include: [
        {
          ...
          where: filters?.dishTypeId
            ? { dishTypeId: filters.dishTypeId }
            : undefined,
   ...
   */
  async getCanteenIngredientsStatistics(
    canteenId: number,
    filters?: {
      date?: string;
      dateRangeStart?: string;
    ...
    for (const recipe of recipes) {
      if (Array.isArray(recipe.ingredients)) {
        recipe.ingredients.forEach((ingId: number) =>
    ...
    for (const record of records) {
      ...
      for (const ingredientId of recipe.ingredients) {
        ...
        const totalQuantityForThisRecord =
    ...
    return {
      ...
      summary: {
        ...
        totalQuantity: ingredientsArray.reduce(
          (sum, ing) => sum + ing.totalQuantity,
          0,
        ),
      },
    };
