...
export class MenuService {
  async createMenu(data: {
    ...
    if (data.finalDate <= data.initialDate)
      throw new Error("FINAL_DATE_MUST_BE_GREATER_THAN_INITIAL_DATE");
  ...
  async getCurrentWeekMenuDetailed(
    menuTypeId?: number,
    weekOffset: number = 0,
  ) {
    ...
    if (dishTypeIds.size) {
      const dTypes = await DishType.findAll({
        where: { id: Array.from(dishTypeIds) },
      });
      for (const dt of dTypes) dishTypeMap.set(dt.id, dt.name);
    }

    const recipeIds = Array.from(
      new Set(dishes.map((d: any) => d.recipeId).filter(Boolean)),
    );
    ...
    for (const r of recipes) {
      if (Array.isArray((r as any).ingredients)) {
        for (const ingId of (r as any).ingredients as number[])
          ingredientIds.add(ingId);
      }
    }

    const ingredients = ingredientIds.size
      ? await Ingredient.findAll({ where: { id: Array.from(ingredientIds) } })
      : [];
    ...
    const nutritionTypeInfoById = new Map<
      number,
      { name: string; unitId?: number }
    >();
    ...
    if (allergenIds.size) {
      const allergens = await Allergen.findAll({
        where: { id: Array.from(allergenIds) },
      });
    ...
    for (const meal of meals) {
          ...
          if (Array.isArray(rawNut) && ingredientGrams && ingredientGrams > 0) {
            for (const n of rawNut as any[]) {
              ...
              if (typeof n?.typeId === "number") typeId = n.typeId;
                ...
                typeof n?.typeId === "string" &&
                !Number.isNaN(Number(n.typeId))
              )
              ...
              const pct =
                typeof n?.percentage === "number"
                  ? n.percentage
                  : typeof n?.percentage === "string" &&
                      !Number.isNaN(Number(n.percentage))
                    ? Number(n.percentage)
                    : 0;
              if (typeId !== null) {
      ...
      if (recipe && Array.isArray((recipe as any).ingredients)) {
        for (const ingId of (recipe as any).ingredients as number[]) {
          ...
          if (unitName === "g") totalGrams += Number(ing.quantity) || 0;
            ...
            totalGrams += (Number(ing.quantity) || 0) * 1000;
        }
      }

      const nutrition = [] as any[];
      for (const [typeId, totalGramsOfNutrient] of Array.from(
        nutritionAgg.entries(),
      )) {
        ...
        if (!name) {
          ...
          if (ntRec) {
            ...
            nutritionTypeInfoById.set(ntRec.id, {
              name: (ntRec as any).name,
              unitId: (ntRec as any).unitId,
            });
          }
        }
        ...
        const unitIdForType =
          info?.unitId || (info === null ? undefined : info.unitId);
        const unitNameForType = unitIdForType
          ? unitById.get(unitIdForType)
          : null;
        ...
        const percentageOfMeal =
          totalGrams > 0 ? (grams / totalGrams) * 100 : null;
        nutrition.push({
          typeId,
          name,
          unitId: unitIdForType || null,
          unit: unitNameForType || "g",
          value: Number((valueInUnit || 0).toFixed(2)),
          grams,
          percentageOfMeal,
        });
      }
      ...
      if (!daysMap[key])
        daysMap[key] = { date: new Date(meal.date), meals: [] };
      const mealEntry: any = {
        id: meal.id,
        name: meal.name,
