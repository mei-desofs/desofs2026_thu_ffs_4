...
export class PerformanceService {
  ...
  async calculateWastePercentage(filter?: {
    ...
    if (
      ...
      const startDate = filter.dateRangeStart
        ? new Date(filter.dateRangeStart)
        : null;
      const endDate = filter.dateRangeEnd
        ? new Date(filter.dateRangeEnd)
        : null;
    ...
    console.log(
      `[PerformanceService] Found ${reservations.length} reservations with consumed/not consumed status`,
    );

    // Agrupar por data para retornar detalhes
    const byDate: Record<
      string,
      { served: number; notConsumed: number; percentage: number }
    > = {};
    ...
    reservations.forEach((reservation: any) => {
      if (!reservation.meal) {
        console.log(
          `[PerformanceService] Reservation ${reservation.id} has no meal`,
        );
        return;
      }
    ...
    console.log(
      `[PerformanceService] Total served: ${totalServed}, Total not consumed: ${totalNotConsumed}`,
    );

    // Calcular percentagem
    const wastePercentage =
      totalServed > 0 ? (totalNotConsumed / totalServed) * 100 : 0;

    // Calcular percentagem por data
    Object.keys(byDate).forEach((dateKey) => {
      const data = byDate[dateKey];
      data.percentage =
        data.served > 0 ? (data.notConsumed / data.served) * 100 : 0;
    });
    ...
    Object.values(byMeal).forEach((mealData) => {
      mealData.percentage =
        mealData.served > 0
          ? (mealData.notConsumed / mealData.served) * 100
          : 0;
    });
    ...
    if (filter?.dayOfWeek !== undefined) {
      ...
      const rangeStart = filter.dateRangeStart
        ? new Date(filter.dateRangeStart)
        : null;
      const rangeEnd = filter.dateRangeEnd
        ? new Date(filter.dateRangeEnd)
        : null;
    ...
    const recalculatedWastePercentage =
      recalculatedTotalServed > 0
