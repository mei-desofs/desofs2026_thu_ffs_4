...
export class WasteReportService {
  ...
  async getWasteReportStatistics(filter?: {
    ...
    filteredReports.forEach((report: any) => {
      ...
      if (!byMeal[mealId]) {
        const mealDate = report.meal?.date
          ? new Date(report.meal.date).toISOString()
    ...
    Object.values(byMeal).forEach((mealData) => {
      const totalWaste = mealData.reports.reduce(
        (sum, r) => sum + r.wastePercentage,
        0,
      );
      mealData.averageWaste =
        mealData.reports.length > 0 ? totalWaste / mealData.reports.length : 0;
    ...
    const averageWaste =
      totalReports > 0
        ? allReports.reduce((sum, r) => sum + r.wastePercentage, 0) /
          totalReports
        : 0;
